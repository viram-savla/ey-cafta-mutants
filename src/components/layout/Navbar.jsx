...
export function Navbar({ activeTab, onTabChange, hedgeValue }) {
  ...
  const [sofrSource, setSofrSource] = useState('NY Fed');
  ...
  useEffect(() => {
    fetch('https://markets.newyorkfed.org/api/rates/sofr/last/1.json')
      ...
      .then(data => {
        ...
        if (rate) { setSofr(parseFloat(rate)); setSofrSource('NY Fed Live'); }
      })
      .catch(() => {});
  }, []);
  ...
  return (
    <nav className="sticky top-0 z-50 border-b" style={{ background: '#090e1c', borderColor: 'var(--border)' }}>
